...
KeyFrame::KeyFrame(const ros::Time& stamp, std::string& robot_ns, const Eigen::Isometry3d& odom, double accum_distance, const pcl::PointCloud<PointT>::ConstPtr& cloud) : stamp(stamp), robot_ns(robot_ns), odom(odom), accum_distance(accum_distance), cloud(cloud), node(nullptr) {}
